...
import { ILoyaltyMember } from "modules/shared/models/loyaltyMember.model";
...
export class OrdersService {
  ...
  checkValidationCoupon(code: string): Observable<ICoupon> {
    ...
    return this.service.get(url).pipe<ICoupon>(
      tap((response: any) => {
        return response;
      })
    );
  ...
  getLoyaltyMemberInfo(): Observable<ILoyaltyMember> {
    let userId = this.identityService.UserData.sub;
    console.log("Userid: " + userId);
    let url = this.ordersUrl + `/cp/api/v1/loyaltymembers/${userId}`;

    return this.service.get(url).pipe<ILoyaltyMember>(
      tap((response: any) => {
  ...
  updateLoyaltyMemberInfo(
    loyaltyMember: ILoyaltyMember
  ): Observable<ILoyaltyMember> {
    let url = this.ordersUrl + `/cp/api/v1/loyaltymembers/update`;

    return this.service.putWithId(url, loyaltyMember).pipe<any>(
      tap(() => {
        return;
      })
    );
